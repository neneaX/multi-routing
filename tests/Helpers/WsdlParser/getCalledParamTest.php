...
class getCalledParamTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * When the request is valid with body and a namespaced method then return param
     */
    public function testWhenTheRequestIsValidWithBodyAndANamespacedMethodThenReturnParam()
    {
        $this->parser->setRequest($this->requestStringNamespaced);
        static::assertEquals('The Fleamarket 1', $this->parser->getCalledParam('BookName'));
    }

    /**
     * When the request is valid with body and a namespaced method but the parameter does not exist then return null
     */
    public function testWhenTheRequestIsValidWithBodyAndANamespacedMethodButTheParameterDoesNotExistThenReturnNull()
    {
        $expected = [
        ...
        $this->parser->setRequest($this->requestStringNamespaced);
        static::assertNull($this->parser->getCalledParam('Pages'));
    }
    ...
    public function testWhenTheRequestIsValidWithBodyAndANonNamespacedMethodThenReturnParams()
    {
        ...
        static::assertEquals('The Fleamarket 2', $this->parser->getCalledParam('BookTitle'));
    }

    /**
     * When the request is valid with body and a non-namespaced method but called parameter doesn't exist then return null
     */
    public function testWhenTheRequestIsValidWithBodyAndANonNamespacedMethodButCalledParameterDoesnTExistThenReturnNull()
    {
        $this->parser->setRequest($this->requestStringNotNamespaced);
        static::assertNull($this->parser->getCalledParam('Cover'));
    }
}
